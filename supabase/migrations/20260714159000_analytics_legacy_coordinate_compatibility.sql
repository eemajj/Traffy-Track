...
-- Forward-fix for legacy CityData rows that stored longitude in `lat` and
-- latitude in `lng`. New imports are canonical, so normalize both layouts at
-- read time without rewriting production ticket data.
...
as $$
  ...
  raw_points as materialized (
    select
      ...
      ticket.state,
      case
        when ticket.lat between 90 and 180 and ticket.lng between -90 and 90
          then ticket.lng::double precision
        else ticket.lat::double precision
      end as lat,
      case
        when ticket.lat between 90 and 180 and ticket.lng between -90 and 90
          then ticket.lat::double precision
        else ticket.lng::double precision
      end as lng,
      ticket.timestamp
    from public.tickets as ticket
  ),
  points as materialized (
    select
      ...
      raw.lng,
      st_transform(
        st_setsrid(st_makepoint(raw.lng, raw.lat), 4326),
        32647
      ) as projected
    from raw_points as raw
    cross join bounds
    where raw.timestamp >= bounds.start_at
      and raw.timestamp < bounds.end_at
      and raw.lat between 13.3 and 14.2
      and raw.lng between 100.2 and 101.1
  ),
