...
export async function createSystemBackupExport() {
  ...
  const snapshotResult = await supabase.rpc("create_system_backup_snapshot");
  if (snapshotResult.error || !snapshotResult.data) {
    throw new Error(`สร้าง consistent database snapshot ไม่สำเร็จ: ${snapshotResult.error?.message || "ไม่พบข้อมูล"}`);
  }
  ...
  const tableEntries = BACKUP_TABLES.map((table) => ({
    table,
    rows: (Array.isArray(snapshotTables[table]) ? snapshotTables[table] : []) as Array<Record<string, unknown>>
  }));
  ...
  const manifest = {
    generatedAt: snapshot.generatedAt || generatedAt.toISOString(),
    snapshotId: snapshot.snapshotId || null,
    snapshotMode: "postgres-single-statement-mvcc-v1",
    ...
    notes: [
      ...
      "ข้อมูลฐานข้อมูลทุกตารางอ่านจาก PostgreSQL MVCC snapshot เดียว",
